 ...
 *       TRUE
 * 
 * Origen: https://github.com/amnesty/drupal-nif-nie-cif-validator
 */

class ValidadorDocumentoId {
    ...
    public function isValidNIF($docNumber) {
        ...
        if (self::isValidNIFFormat($fixedDocNumber)) {
            ...
            $isValid = ($writtenDigit == $correctDigit); 
        }
        return $isValid;
    }

    public function getValidNIFLetter($docNumber) {
        $isValid = FALSE;
        $fixedDocNumber = "";
        $correctDigit = "";
        $writtenDigit = "";
        if (!preg_match("/^[A-Z]+$/i", substr($fixedDocNumber, 1, 1))) {
            $fixedDocNumber = strtoupper(substr($docNumber."000000000", 1,9));
        } else {
            $fixedDocNumber = strtoupper($docNumber);
        }
        $writtenDigit = strtoupper(substr($docNumber, -1, 1));
        ...
        return json_encode([$fixedDocNumber,$writtenDigit,$correctDigit]);
    }
    /*
     *   This function validates a Spanish identification number
     *   verifying its check digits.
    ...
    public function isNotValidCIF($docNumber) {
        return !self::isValidCIF($docNumber);
    }
    ...
    public function getNIFCheckDigit($docNumber) {
        ...
        if (self::isValidNIFFormat($fixedDocNumber)) {
            ...
            if (self::isValidNIFFormat($fixedDocNumber)) {
                ...
                \Illuminate\Support\Facades\Log::debug('letra',['doc'=>$fixedDocNumber,'posicion'=>$position,'letra'=>$correctLetter]);                
            }
        }
    ...
    protected function getCIFCheckDigit($docNumber) {
        ...
        if (self::isValidCIFFormat($fixedDocNumber)) {
            ...
            $oddSum = self::sumDigits(substr($centralChars, 0, 1) * 2) +
    ...
    function getDocTypeId($docNumber) {
        $docType = ['id'=> "", 'descripcion'=> 'error'];
        $firstChar = substr($docNumber, 0, 1);

        if (self::isValidNIFFormat($docNumber) or
                self::isValidNIEFormat($docNumber) or
                self::isValidCIFFormat($docNumber)) {
            $docType =['id'=> $firstChar, 'descripcion'=> $this->identificationType[$firstChar]];
        }
        return $docType;
    }
