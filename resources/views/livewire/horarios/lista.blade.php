<x-botonera-page>
    ...
    <x-slot name="botonera">
        <x-button type="button" class="btn btn-nuevo" id="btn-nuevo-horario"
            wire:click="$dispatch('abrir-crear-horario')" title="{{ __('Nuevo') }}">
                        ...
                        @foreach ($items as $item)
                            <tr wire:key="{{ $item->id }}">
                                ...
                                <td class="px-4 whitespace-nowrap">
                                    <x-button type="button" class="btn-editar" id="btn-editar-horario-{{ $item->id }}"
                                        wire:click="$dispatch('horario-editar', {id: {{ $item->id }}})"
                                        title="{{ __('Modificar') }}">
                                    ...
                                    @if (in_array($item->id, $idsAccesibles))
                                        @include('livewire.parciales.boton-ficha-inicio', [
                                            'id'  => $item->id,
                                            'url' => route('horario.entrar', $item, false),
                                        ])
                                    @endif
