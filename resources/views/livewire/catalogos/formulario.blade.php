<x-dosl.dialog-modal wire:model.live="abrir" class="backdrop-blur" maxWidth="lg">
    <x-slot name="title">
        {{ $itemId ? __('Modificar') : __('Nuevo') }}
    </x-slot>

    <x-slot name="content">
        <div>
            <x-label for="cat-descripcion" :value="__('Descripción')" />
            <x-input id="cat-descripcion" class="block mt-1 w-full" type="text" wire:model="descripcion" autofocus />
