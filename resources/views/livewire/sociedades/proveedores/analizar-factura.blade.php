<x-dosl.dialog-modal wire:model.live="abrir" class="backdrop-blur" maxWidth="2xl">
    ...
    <x-slot name="content">
        <label for="input-facturas-sociedad"
            x-data="{ arrastrando: false }"
            x-on:dragover.prevent="arrastrando = true"
            x-on:dragleave.prevent="arrastrando = false"
            x-on:drop.prevent="arrastrando = false; $wire.uploadMultiple('facturas', $event.dataTransfer.files, () => {}, (error) => console.error('uploadMultiple: error', error))"
            ...
            <input type="file" id="input-facturas-sociedad" wire:model="facturas" multiple
        ...
        @if (count($facturas))
            <ul class="mt-4 divide-y border rounded">
                @foreach ($facturas as $index => $factura)
                    <li class="flex items-center justify-between px-3 py-2 text-sm" wire:key="factura-sociedad-{{ $index }}">
                        <span class="truncate">
