        ...
        @if ($formulario->proveedor?->exists || $formulario->documentoComprobado)
            <div class="mt-2 w-1/2">
                ...
                <x-select id="select-proveedor-tipo" class="block mt-1 w-full mayusculas" name="tipo-proveedor"
                    ...
                    <option value="">{{ __('--') }}</option>
                    <optgroup label="{{ __('Compras (400)') }}">
                        @foreach ($tiposProveedor->filter(fn ($tipo) => $tipo->esDeCompras()) as $tipo)
                            <option value="{{ $tipo->id }}">{{ $tipo->descripcion }}</option>
                        @endforeach
                    </optgroup>
                    <optgroup label="{{ __('Servicios exteriores (410)') }}">
                        @foreach ($tiposProveedor->reject(fn ($tipo) => $tipo->esDeCompras()) as $tipo)
                            <option value="{{ $tipo->id }}">{{ $tipo->descripcion }}</option>
                        @endforeach
                    </optgroup>
                </x-select>
