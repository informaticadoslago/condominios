<x-botonera-page>
    ...
    <x-slot name="subtitulo">
        {{ __('Empresas y sociedades') }}
    </x-slot>
    <x-slot name="botonera">
        <x-button type="button" class="btn btn-nuevo" id="btn-nuevo-sociedad"
            wire:click="$dispatch('abrir-crear-sociedad')" title="{{ __('Nuevo') }}">
    ...
    <x-slot name="content">
                        ...
                        @foreach ($items as $item)
                            <tr wire:key="{{ $item->id }}">
                                ...
                                <td class="px-4 whitespace-nowrap">
                                    @if ($item->estado_id == \App\Models\Sociedad::ESTADO_ACTIVO)
                                        <x-button type="button" class="btn-editar" id="btn-editar-sociedad-{{ $item->id }}"
                                            wire:click="$dispatch('sociedad-editar', {id: {{ $item->id }}})"
                                            title="{{ __('Modificar') }}">
                                        ...
                                        @if (in_array($item->id, $idsAccesibles))
                                            @include('livewire.parciales.boton-ficha-inicio', [
                                                'id'  => $item->id,
                                                'url' => route('sociedad.entrar', $item, false),
                                            ])
                                        @endif
        ...
        @livewire('sociedades.formulario')
    </x-slot>
</x-botonera-page>
