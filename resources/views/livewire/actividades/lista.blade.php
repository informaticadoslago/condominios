<x-botonera-page>
    ...
    <x-slot name="subtitulo">
        {{ __('Actividades de la comunidad') }}
    </x-slot>
    <x-slot name="botonera">
        <x-button type="button" class="btn btn-nuevo" id="btn-nuevo-actividad"
            wire:click="$dispatch('abrir-crear-actividad')" title="{{ __('Nuevo') }}">
    ...
    <x-slot name="content">
                        ...
                        @foreach ($items as $item)
                            <tr wire:key="{{ $item->id }}">
                                <td class="px-6 py-4">
                                    ...
                                    @if ($item->proyecto_contable_id)
                                        <i class="fa-solid fa-link ml-1 text-green-600"
                                            title="{{ __('Enlazada con la contabilidad') }}"></i>
                                    @endif
                                </td>
                                <td class="px-4 whitespace-nowrap">
                                    <x-button type="button" class="btn-editar" id="btn-editar-actividad-{{ $item->id }}"
                                        wire:click="$dispatch('actividad-editar', {id: {{ $item->id }}})"
                                        title="{{ __('Modificar') }}">
        ...
        @livewire('actividades.formulario')
    </x-slot>
</x-botonera-page>
