<x-botonera-page>
    <x-slot name="title">
        {{ __('Entidades financieras') }}
    </x-slot>
    <x-slot name="subtitulo">
        {{ __('Bancos y entidades de crédito') }}
    </x-slot>
    <x-slot name="botonera">
        <x-button type="button" class="btn btn-nuevo" id="btn-nuevo-entidad-bancaria"
            wire:click="$dispatch('abrir-crear-entidad-bancaria')" title="{{ __('Nuevo') }}">
    ...
    <x-slot name="content">
        <x-dosl.tabla>
            <div class="py-3 px-6 flex items-center">
                ...
                @include('livewire.parciales.buscador', ['placeholder' => "Descripción, código o BIC"])
            </div>
            @if (count($items))
                <table class="table-striped w-full table-auto text-sm text-left">
                    <thead class="font-medium border-b">
                        <tr>
                            ...
                            <th class="cursor-pointer py-3 px-6" wire:click="ordenar('bic')">
                                {{ __('BIC') }}
                                @if ($sort == 'bic')
                                    <i class="fa-solid fa-sort-{{ $direction == 'asc' ? 'up' : 'down' }} float-right mt-1"></i>
                                @else
                    ...
                    <tbody class="divide-y">
                        @foreach ($items as $item)
                            <tr wire:key="{{ $item->id }}">
                                ...
                                <td class="px-6 py-4">
                                    <span class="mayusculas">{{ $item->descripcion }}</span>
                                </td>
                                <td class="px-6 py-4">{{ $item->bic }}</td>
                                <td class="px-6 py-4">{{ $item->estado?->descripcion }}</td>
                                <td class="px-4 whitespace-nowrap">
                                    @if ($item->estado_id == \App\Models\EntidadBancaria::ESTADO_ACTIVO)
                                        <x-button type="button" class="btn-editar" id="btn-editar-entidad-bancaria-{{ $item->id }}"
                                            wire:click="$dispatch('entidad-bancaria-editar', {id: {{ $item->id }}})"
                                            title="{{ __('Modificar') }}">
        ...
        @livewire('maestros.entidades-bancarias.formulario')
    </x-slot>
</x-botonera-page>
