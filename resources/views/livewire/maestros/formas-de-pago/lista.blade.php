<x-botonera-page>
    ...
    <x-slot name="botonera">
        <x-button type="button" class="btn btn-nuevo" id="btn-nuevo-forma-de-pago"
            wire:click="$dispatch('abrir-crear-forma-de-pago')" title="{{ __('Nuevo') }}">
    ...
    <x-slot name="content">
                        ...
                        @foreach ($items as $item)
                            <tr wire:key="{{ $item->id }}">
                                ...
                                <td class="px-4 whitespace-nowrap">
                                    @if ($item->estado_id == \App\Models\FormaDePago::ESTADO_ACTIVO)
                                        <x-button type="button" class="btn-editar" id="btn-editar-forma-de-pago-{{ $item->id }}"
                                            wire:click="$dispatch('forma-de-pago-editar', {id: {{ $item->id }}})"
                                            title="{{ __('Modificar') }}">
        ...
        @livewire('maestros.formas-de-pago.formulario')
    </x-slot>
</x-botonera-page>
