{{-- Recibos de una remesa: A4 vertical. --}}
@php
...
<head>
    <meta charset="utf-8">
    <title>{{ __('Remesa') }} {{ $remesa->referencia }}</title>
    <style>
    ...
    <table class="maq">
        <tr>
            ...
            <td class="centro" width="50%">
                <h1>{{ __('Remesa') }} {{ $remesa->referencia }}</h1>
            </td>
            <td width="25%"></td>
        </tr>
    </table>
...
<div class="meta">
    <strong>{{ __('Fecha de cargo') }}</strong>: {{ $remesa->fecha_cargo?->format('d/m/Y') }}
    &nbsp;·&nbsp;{{ __('Cuenta') }}: {{ $remesa->cuentaBancaria?->iban }}
</div>
...
<table>
    <thead>
        <tr>
            ...
            <th class="izq">{{ __('Propietario') }}</th>
    ...
    <tbody>
        @forelse ($lineas as $linea)
            <tr>
                ...
                <td class="izq">{{ $linea->recibo?->propietario?->persona?->nombreCompleto }}</td>
                <td class="der">{{ $fmt($linea->recibo?->importe) }}</td>
            </tr>
        @empty
            <tr>
                <td class="izq" colspan="3">{{ __('Esta remesa no tiene recibos.') }}</td>
            </tr>
        @endforelse
    </tbody>
    <tfoot>
