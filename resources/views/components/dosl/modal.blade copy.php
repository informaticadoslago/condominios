...
<div x-data="{
    ...
    {{-- Dialog --}}
    <div x-show="show" x-ref="dialog" :class="draggable ? 'fixed' : 'relative sm:mx-auto'"
        :style="draggable
            ?
            ...
            ({{ $modal2l ? 'true' : 'false' }} && '{{ $maxWidth }}'
                === 'sm:max-w-full' ? 'width:' + (window.innerWidth - 56 - 16) + 'px;height:' + (window.innerHeight -
                    256 - 16) + 'px;' : '')"
        class="mb-6 bg-white dark:bg-gray-800 rounded-lg shadow-xl transform transition-all {{ $maxWidth }} flex flex-col overflow-hidden"
        ...
        <div class="px-4 py-4 overflow-y-auto" style="max-height: calc(100vh - 140px);" x-ref="body">
            {{ $slot }}
        </div>
    </div>
</div>
